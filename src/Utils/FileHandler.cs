...
public static class FileHandler
{
    ...
    {
        return File.ReadAllText(BaseDirectory + $"/{path}");
    }

    public static byte[] ReadBytes(string path)
    {
        return File.ReadAllBytes(BaseDirectory + $"/{path}");
    }
    ...
    {
        return File.Exists(BaseDirectory + $"/{path}");
    }
            ...
            return Directory.GetFiles(fullPath)
                .Select(file =>
                    Path.Combine(directoryPath,
                        Path.GetFileName(file))) // Combine with directoryPath for relative paths
                .ToList();
